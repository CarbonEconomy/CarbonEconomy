...
const updateUser = async (txn, userID, newCredit) => {
    const statement = `UPDATE ${tableName} AS gu SET gu.totalCredit = ${newCredit} WHERE gu.id = ${parseInt(userID)}`;
    let result = await txn.execute(statement);
    return result;
}
...
const updateUserCredit = async (userID, creditToAdd) => {
    ...
    await qldbDriver.executeLambda(
        async (txn) => {
            ...
            const newCredit = parseInt(resultList[0].get("totalCredit")) + parseInt(creditToAdd);
